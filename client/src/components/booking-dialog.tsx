...
const bookingFormSchema = insertBookingSchema.extend({
  seatsBooked: z.number().min(1, "Select at least 1 seat"),
});
...
export function BookingDialog({ open, onOpenChange, car }: BookingDialogProps) {
  ...
  const calculateFare = () => {
    ...
    if (tripType === "round_trip") {
      return baseFare + (car.returnFare * seatsBooked);
    }
    return baseFare;
  };
        ...
        <DialogContent className="max-w-md">
          <div className="py-6 text-center">
            ...
            <p className="text-muted-foreground text-sm mb-4">
              Your ride has been booked successfully. The driver will contact you soon.
            </p>
            <div className="bg-muted/50 rounded-lg p-4 text-left mb-4">
              <div className="flex items-center gap-2 text-sm mb-2">
                ...
                <span>{car.destination}</span>
              </div>
              <div className="flex items-center gap-2 text-sm mb-2">
                <Car className="h-4 w-4 text-muted-foreground" />
                <span>{car.carModel} - {car.driverName}</span>
              </div>
              <div className="flex items-center gap-2 text-sm">
                <Clock className="h-4 w-4 text-muted-foreground" />
                <span>Departure: {car.departureTime}</span>
              </div>
            </div>
            <p className="font-semibold text-lg mb-4">
              Total: ${calculateFare()}
            </p>
            <Button onClick={handleClose} className="w-full" data-testid="button-close-booking-success">
              Done
            </Button>
          </div>
        </DialogContent>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="+1234567890" {...field} data-testid="input-customer-phone" />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {car.seatsAvailable} seats available
                  </p>
                </FormItem>
              )}
            />
                    ...
                    >
                      <div>
                        ...
                        >
                          ...
                          <span className="text-lg font-bold">${car.fare * seatsBooked}</span>
                        </Label>
                      </div>
                      <div>
                        <RadioGroupItem
                        ...
                        >
                          ...
                          <span className="text-lg font-bold">${(car.fare + car.returnFare) * seatsBooked}</span>
                        </Label>
                      </div>
                    </RadioGroup>
                  </FormControl>
