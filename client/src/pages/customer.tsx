...
export default function Customer() {
  ...
  const filteredCars = availableCars.filter((car) => {
    const matchOrigin = searchOrigin
      ? car.origin.toLowerCase().includes(searchOrigin.toLowerCase())
      : true;
    const matchDestination = searchDestination
      ? car.destination.toLowerCase().includes(searchDestination.toLowerCase())
      : true;
          ...
          <div className="space-y-4">
            ...
            <div className="grid gap-4">
              {filteredCars.map((car) => (
                <Card key={car.id} className="hover-elevate" data-testid={`card-ride-${car.id}`}>
                  <CardContent className="p-4">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground mb-3">
                          ...
                          <div className="flex items-center gap-1.5">
                            ...
                            <span>{car.seatsAvailable} seat{car.seatsAvailable !== 1 ? "s" : ""}</span>
                          </div>
                        </div>
                        <div className="flex items-center gap-4 text-sm">
                          <div className="flex items-center gap-1.5">
                            <Clock className="h-4 w-4 text-muted-foreground" />
                            <span>Departs: <span className="font-medium">{car.departureTime}</span></span>
                          </div>
                          <div className="flex items-center gap-1.5">
                            <Clock className="h-4 w-4 text-muted-foreground" />
                            <span>Returns: <span className="font-medium">{car.returnTime}</span></span>
                          </div>
                        </div>
                      </div>
                      <div className="flex flex-col items-end gap-3">
                        <div className="text-right">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-bold text-lg">${car.fare}</span>
                          </div>
                          <div className="flex items-center gap-2 mt-1">
                            <Badge variant="outline">Round Trip</Badge>
                            <span className="font-semibold text-muted-foreground">${car.fare + car.returnFare}</span>
                          </div>
                        </div>
                        <Button 
