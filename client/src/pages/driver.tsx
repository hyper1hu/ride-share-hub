...
export default function Driver() {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        title: "Car removed",
        description: "Your car listing has been removed successfully.",
      });
      setDeleteCarId(null);
    },
    onError: () => {
      toast({
        title: "Error",
        description: "Failed to remove car listing. Please try again.",
        variant: "destructive",
      });
    },
  });
  ...
  const getCarBookings = (carId: string) => {
    return bookings.filter((b) => b.carId === carId && b.status !== "cancelled");
  };
  ...
  const getStatusBadge = (status: CarType["status"]) => {
    switch (status) {
      ...
      case "on_trip":
        return <Badge className="bg-blue-500/10 text-blue-600 dark:text-blue-400 border-blue-500/20">On Trip</Badge>;
      case "returning":
        return <Badge className="bg-orange-500/10 text-orange-600 dark:text-orange-400 border-orange-500/20">Returning</Badge>;
      default:
        return null;
    }
  };
  ...
  return (
    <div className="min-h-screen bg-background">
          ...
          <Card>
            <CardContent className="py-16 text-center">
              ...
              <p className="text-muted-foreground text-sm max-w-sm mx-auto mb-4">
                Start earning by listing your car. Set your route, timings, and fare.
              </p>
              <Button onClick={() => setAddCarOpen(true)} className="gap-2" data-testid="button-add-car-empty">
                <Plus className="h-4 w-4" />
                Add Your First Car
              </Button>
            </CardContent>
                    ...
                    <div className="grid sm:grid-cols-2 gap-4">
                      <div className="space-y-3">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-sm">
                            Departs <span className="font-medium">{car.departureTime}</span>, Returns <span className="font-medium">{car.returnTime}</span>
                          </span>
                        </div>
                        <div className="flex items-center gap-3">
                          <Phone className="h-4 w-4 text-muted-foreground" />
                          <span className="text-sm">{car.driverPhone}</span>
                        </div>
                      </div>
                      <div className="space-y-3">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-sm">
                            <span className="font-medium">{car.seatsAvailable - totalSeatsBooked}</span> of {car.seatsAvailable} seats available
                          </span>
                        </div>
                        <div className="flex items-center gap-4 text-sm">
                          <div>
                            <span className="text-muted-foreground">One Way:</span>{" "}
                            <span className="font-semibold">${car.fare}</span>
                          </div>
                          <div>
                            <span className="text-muted-foreground">Round Trip:</span>{" "}
                            <span className="font-semibold">${car.fare + car.returnFare}</span>
                          </div>
                        </div>
      ...
      <AlertDialog open={!!deleteCarId} onOpenChange={() => setDeleteCarId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Remove car listing?</AlertDialogTitle>
            <AlertDialogDescription>
              This will remove your car listing and any pending bookings. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel data-testid="button-cancel-delete">Cancel</AlertDialogCancel>
            <AlertDialogAction
              onClick={() => deleteCarId && deleteMutation.mutate(deleteCarId)}
